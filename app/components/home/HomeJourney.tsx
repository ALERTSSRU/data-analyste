...
export function HomeJourney({
  ...
  return (
    <main className="overflow-x-hidden">
      ...
      <section data-scene="school" className="relative mx-auto max-w-6xl px-4 sm:px-6 py-16 sm:py-24 md:py-28">
        <ScrollReveal>
        ...
        <div className="mt-8 sm:mt-12 grid gap-5 md:grid-cols-2">
          {education.map((item) => (
            <ScrollReveal key={`${item.school_name}-${item.degree}`}>
              <article className="glass-panel rounded-3xl border p-5 sm:p-7 shadow-lg" style={{ borderColor: 'var(--panel-border)' }}>
      ...
      <section data-scene="bank" className="relative mx-auto max-w-6xl px-4 sm:px-6 py-16 sm:py-24 md:py-28">
        <ScrollReveal>
        ...
        <div className="mt-8 sm:mt-12 space-y-5">
          {featuredBank.map((item) => (
            <ScrollReveal key={`${item.company}-${item.position}`}>
              <article className="glass-panel rounded-3xl border border-emerald-400/20 p-5 sm:p-7 md:p-8 shadow-xl">
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
                  <div>
                    ...
                    <p className="mt-1 text-base sm:text-lg" style={{ color: 'var(--foreground-muted)' }}>
                ...
                <p className="mt-4 sm:mt-5 max-w-3xl text-xs sm:text-sm leading-relaxed" style={{ color: 'var(--foreground-muted)' }}>
                  <Trans>{item.description}</Trans>
                </p>
              </article>
            </ScrollReveal>
        ...
        {otherRoles.length ? (
          <div className="mt-6 sm:mt-8 grid gap-4 sm:grid-cols-2">
            {otherRoles.map((item) => (
              <article key={`${item.company}-${item.position}`} className="glass-panel rounded-2xl border p-4 sm:p-5" style={{ borderColor: 'var(--panel-border)' }}>
      ...
      <section className="relative mx-auto max-w-6xl px-4 sm:px-6 py-14 sm:py-20">
        <ScrollReveal>
          ...
          <div className="mt-5 sm:mt-6 flex flex-wrap gap-2">
            {skills.slice(0, 18).map((skill) => (
              <span
                ...
                className="rounded-full border px-3.5 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm shadow-xs transition-all hover:border-cyan-400/40"
                style={{
      ...
      <section className="relative mx-auto max-w-6xl px-4 sm:px-6 py-16 sm:py-24 md:py-28">
        <ScrollReveal>
        ...
        <section className="relative mx-auto max-w-6xl px-4 sm:px-6 py-12 sm:py-16">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
            {certifications.slice(0, 3).map((cert) => (
              <article key={cert.title} className="glass-panel rounded-2xl border p-4 sm:p-5 shadow-sm" style={{ borderColor: 'var(--panel-border)' }}>
      ...
      <section data-scene="finale" className="relative mx-auto max-w-6xl px-4 sm:px-6 py-20 sm:py-28 md:py-32">
        <ScrollReveal>
          <div className="glass-panel rounded-3xl sm:rounded-4xl border border-violet-400/20 p-6 sm:p-10 md:p-16 text-center shadow-2xl">
            ...
            <p className="mx-auto mt-4 sm:mt-5 max-w-2xl text-base sm:text-lg leading-relaxed sm:leading-8" style={{ color: 'var(--foreground-muted)' }}>
